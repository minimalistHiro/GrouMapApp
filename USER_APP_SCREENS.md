...
- 構成: ヘッダー画像、クーポン概要、期限/割引/必要スタンプ（`stamp_reward` の場合は「スタンプカード達成で獲得したクーポンです」達成バッジに置換）、残り枚数（無制限クーポンの場合は非表示）、店舗情報、注意事項、下部固定「使用する」ボタン（使用済み/期限切れ/スタンプ不足/配布終了時は無効化。`stamp_reward` はスタンプ数チェックをスキップ）
- 説明: クーポンの詳細表示・直接使用画面。確認ダイアログ後にFirestoreトランザクションでクーポン使用処理を実行。コイン交換クーポン（`type=coin_exchange`）・スタンプ達成クーポン（`type=stamp_reward`）は `user_coupons` を直接更新する専用フローで処理（トランザクション不使用）。`stamp_reward` は閲覧数インクリメントをスキップし、有効期限なし（`noExpiry=true`）で使用可能
...
- 説明: コイン獲得ミッションの一覧画面。Firestore連携でミッション達成状態をリアルタイム反映し、「受け取る」タップでコインをDBに加算。デイリーミッションは日付ベースで自動リセット。コイン交換タブでは10コインで未訪問店舗の100円引きクーポンを取得可能（有効期限30日、交換候補は `isActive=true` かつ `isApproved=true` かつ `stores.isOwner!=true` の未訪問店舗のみ）。交換成功後は所持コインと未訪問店舗リストをローカル状態で即時更新してUIに反映し、バックグラウンドでFirestoreから最新データを再取得。新規登録ミッション（5種）をすべて受取完了するまでデイリー・ログインタブはロックされる。ホーム画面右下のフローティングボタンから遷移
...
- 構成: よくある質問（ExpansionTile形式・6件の一般FAQ＋「退会するには？」FAQ）、問い合わせ導線（メールサポート・電話サポート・ライブチャット）。「退会するには？」展開時は案内テキスト＋「退会するにはこちら」赤テキストボタンを表示
- 説明: ヘルプ・サポートの入口画面。退会フロー（`AccountDeletionReasonView`→`AccountDeletionProcessingView`）の入り口もここに集約
