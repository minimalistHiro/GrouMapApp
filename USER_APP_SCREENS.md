...
- 構成: AppBar、コイン残高表示（Firestoreリアルタイム取得）、PillTabBar（デイリー/ログイン/新規登録の3タブ）、ミッションリスト（3状態: 未達成=グレイアウト / 達成済み未受取=ティール〜グリーングラデーション背景・タップで報酬受取 / 受取済み=半透明グラデーション+チェックマーク）
- 説明: コイン獲得ミッションの一覧画面。Firestore連携でミッション達成状態をリアルタイム反映し、「受け取る」タップでコインをDBに加算。デイリーミッションは日付ベースで自動リセット。ホーム画面右下のフローティングボタンから遷移
