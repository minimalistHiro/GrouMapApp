...
- 構成: ログアウト時のみログイン/新規登録カード、現在地に近いおすすめ店舗カード（タイトル無し・タップで店舗詳細へ遷移）、統計カプセルバー（コイン・バッジ・スタンプの保有数をメニュー上部に横並びカプセルで表示、コインはサンプル値・バッジ/スタンプはFirestoreリアルタイム取得）、メニュー、くじ引きキャンペーンボタン（キャンペーン期間中のみ表示・カプセル型・終了日付き）、ニュースセクション（掲載期間内・画像のみ1:1横スクロール・最大7件）、クーポン/投稿セクション（Instagram公開投稿）、各詳細への導線
...
- 構成: AppBar、準備中テキスト
- 説明: くじ引き画面（キャンペーン期間中にホーム画面のボタンから遷移）
