...
- 構成: スタンプ進捗カード（全店舗合計に対する保有スタンプのゲージ/開拓済み/スタンプ満了）、現在地に近いおすすめ店舗カード、クーポン/投稿/店舗/スタンプ等のセクション、各詳細への導線
- 説明: 主要情報と未訪問店舗のレコメンドを集約したダッシュボード

### MapView (`lib/views/map/map_view.dart`)
- 構成: GoogleMap、店舗マーカー、フィルター（営業中/ジャンル/開拓）、店舗情報パネル
- 説明: 周辺店舗を地図で探す画面

### PostsView (`lib/views/posts/posts_view.dart`)
- 構成: 投稿グリッド、空/エラー/読み込み状態
- 説明: 店舗投稿を一覧で閲覧する画面
...
- 構成: タブ表示（店舗情報/投稿/クーポンなど）、お気に入り操作、スタンプ状況
...
- 構成: プロフィール表示、設定/サポート/規約/退会などの導線
- 説明: ユーザーアカウントのハブ画面
