...
- 構成: ログアウト時のみログイン/新規登録カード、現在地に近いおすすめ店舗カード（タイトル無し・タップで店舗詳細へ遷移）、統計カプセルバー（コイン・バッジ・スタンプの保有数をメニュー上部に横並びカプセルで表示、全てFirestoreリアルタイム取得）、メニュー、スロットキャンペーンボタン（キャンペーン期間中かつ当日未プレイ時のみ表示・カプセル型・終了日付き・プレイ済みの日は非表示）、ニュースセクション（掲載期間内・画像のみ1:1横スクロール・最大7件）、クーポン/投稿セクション（Instagram公開投稿）、各詳細への導線、ミッションフローティングボタン（右下・ティール〜グリーングラデーション・ミッション画面へ遷移）
...
- 構成: AppBar、コイン残高表示、3リールスロットマシン（手動停止式）、停止ボタン×3、スタートボタン、当選ポップアップ（1等ゴールド/2等シルバー/ハズレグレー）、紙吹雪アニメーション
- 説明: 1日1回1コインで実行できるスロット画面。確率ベース判定（1等1%/2等5%/ハズレ94%）で結果を事前決定し、スロットアニメーションで演出。キャンペーン期間中にホーム画面のボタンから遷移
