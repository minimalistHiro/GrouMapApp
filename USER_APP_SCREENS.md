...
- 構成: ログアウト時のみログイン/新規登録カード、現在地に近いおすすめ店舗カード（タイトル無し・タップで店舗詳細へ遷移）、統計カプセルバー（コイン・バッジ・スタンプの保有数をメニュー上部に横並びカプセルで表示、全てFirestoreリアルタイム取得）、メニュー、スロットキャンペーンボタン（キャンペーン期間中のみ表示・カプセル型・終了日付き）、ニュースセクション（掲載期間内・画像のみ1:1横スクロール・最大7件）、クーポン/投稿セクション（Instagram公開投稿）、各詳細への導線、ミッションフローティングボタン（右下・ティール〜グリーングラデーション・ミッション画面へ遷移）
