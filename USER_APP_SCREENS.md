...
- 構成: ログアウト時のみログイン/新規登録カード、現在地に近いおすすめ店舗カード（タイトル無し・タップで店舗詳細へ遷移）、統計カプセルバー（コイン・バッジ・スタンプの保有数をメニュー上部に横並びカプセルで表示、コインはサンプル値・バッジ/スタンプはFirestoreリアルタイム取得）、メニュー、スロットキャンペーンボタン（キャンペーン期間中のみ表示・カプセル型・終了日付き）、ニュースセクション（掲載期間内・画像のみ1:1横スクロール・最大7件）、クーポン/投稿セクション（Instagram公開投稿）、各詳細への導線、ミッションフローティングボタン（右下・ティール〜グリーングラデーション・ミッション画面へ遷移）
...
- 構成: AppBar、コイン残高表示、PillTabBar（デイリー/ログイン/新規登録の3タブ）、ミッションリスト（達成済み=ティール〜グリーングラデーション背景・タップでコイン獲得ポップアップ / 未達成=グレイアウト）
- 説明: コイン獲得ミッションの一覧画面（UIのみ、機能未搭載）。ホーム画面右下のフローティングボタンから遷移
