...
開拓率は **個人開拓率** と **コミュニティ開拓率** の2軸を持つ。UI上でトグル切り替えにより表示を変更する。

### 2-1. 個人開拓率（マイ開拓率）

```
個人エリア開拓率 = そのエリアの「自分が訪問済みの店舗数」/ そのエリアの「総店舗数」× 100
```
...
### 2-2. コミュニティ開拓率（みんなの開拓率）

```
コミュニティエリア開拓率 = そのエリアの「discoveredCount >= 1 の店舗数」/ そのエリアの「総店舗数」× 100
```

**判定:** `stores/{storeId}.discoveredCount >= 1`（1人以上がNFCチェックイン済み）

**計算の流れ:**
1. `areas` コレクション全件取得（`isActive=true`）
2. `stores` コレクション取得（`isActive=true && isApproved=true`）
3. エリアごとに `stores.filter(s => s.areaId == area.areaId)` で総数を計算
4. `stores.filter(s => s.areaId == area.areaId && (s.discoveredCount ?? 0) >= 1)` で開拓済み数を計算
5. クライアント側で計算し保持

**注意:** `discoveredCount` は `block-a-cloud-functions.md` で追加されるフィールド（NFCチェックイン初回来店時にインクリメント）。

### 2-3. UI トグル設計

エリア開拓率画面・図鑑タブのサマリーバーで以下のトグルを提供する:

```
[ マイ開拓率 ]  [ みんなの開拓率 ]
```

- **マイ開拓率**: 自分がどれだけ探索したかを確認する用途
- **みんなの開拓率**: コミュニティ全体でそのエリアがどれだけ開拓されているかを示す協力型指標
