...
> **最終更新**: 2026-02-23（メールアドレス変更機能追加・プロフィール完成度2段階化・興味カテゴリ設定画面分離・コイン運用方針の文言整合・コイン有効期限処理の実装反映・スタンプカード単枚運用/常連向けクーポン廃止の方針整合・規約画面を最新Markdownへ同期・問い合わせ先統一・ユーザー用アプリUI基盤統一・プラン契約情報画面Firestore連携/ステータス別表示/無料期間全機能開放対応）
>
...
| **ゲストモード** | ログインなしでもホーム・マップ・投稿の閲覧が可能（ミッションフローティングボタンは非表示） |

### 1.2 ホーム画面

| 機能 | 説明 |
|------|------|
| **おすすめ店舗カード** | 現在地・好みのカテゴリ・エリア・訪問状況を考慮してスコアリングされた店舗をレコメンド表示。`stores.isOwner=true` の店舗は除外 |
...
| **統計カプセルバー** | コイン・バッジ・スタンプの現在数をメニュー上部にカプセル形式で横並び表示（Firestoreリアルタイム取得、バッジ数はローカル定義に存在するバッジのみカウント、未ログイン時は `-` 表示） |
| **スロットキャンペーンボタン（廃止）** | ※スロット機能廃止に伴い削除。コードはコメントアウトで保持 |
| **ミッションフローティングボタン** | ログインユーザーのみ表示。受取可能なミッション（達成済み＆未受取）がある場合はティール〜グリーングラデーション、ない場合はグレーアウト表示。新規登録ミッション未完了時はデイリー・ログインボーナスの受取判定をスキップし誤点灯を防止。ミッション画面から戻った時に状態を再判定 |
...
| `requestEmailOtp` | メール認証用OTPの送信（新規登録時） |
| `verifyEmailOtp` | メール認証用OTPの検証（新規登録時） |
| `requestEmailChangeOtp` | メールアドレス変更用OTPの送信（新メールアドレス宛） |
