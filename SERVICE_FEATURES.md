...
> **最終更新**: 2026-02-22（スロット確率設計を友人案A（期待値0.94）に変更）
>
...
| 機能 | 説明 |
|------|------|
| **おすすめ店舗カード** | 現在地・好みのカテゴリ・エリア・訪問状況を考慮してスコアリングされた店舗をレコメンド表示 |
...
| **デイリーレコメンド** | その日初めてのアプリ起動時におすすめ店舗を最大3件自動表示。スコアリングアルゴリズム（カテゴリ一致+2/エリア一致+1/未訪問+2/訪問済み−1/1km以内+3/3km以内+2/5km以内+1）で上位を選定。候補はstoresコレクションから最大50件取得（isActive=true, isApproved=true）。インプレッション・クリックをFirestoreに記録 |
| **統計カプセルバー** | コイン・バッジ・スタンプの現在数をメニュー上部にカプセル形式で横並び表示（Firestoreリアルタイム取得、バッジ数はローカル定義に存在するバッジのみカウント、未ログイン時は `-` 表示） |
| **スロットキャンペーンボタン（一時非表示）** | ※現在一時的に非表示。ログインユーザーのみ、キャンペーン期間中かつ当日未プレイ時のみメニュー下にカプセル型画像バナーボタンを表示、プレイ済みの日は非表示、タップでスロット画面へ遷移 |
...
| **表示対象店舗条件** | `isActive=true` かつ `isApproved=true` の店舗のみ表示し、`stores.isOwner=true` または `stores.createdBy` の `users.isOwner=true` に該当する店舗は除外 |
...
| 機能 | 説明 |
|------|------|
| **バッジ一覧** | 全141種のバッジをグリッド表示。Firestoreの `user_badges` から取得済みバッジを判定し、取得済みは画像・名前を通常表示、未取得は「？」アイコン・「？？？」名前で表示。ヘッダー右上フィルターでカテゴリ絞り込み可能 |
...
| **スロット（一時非表示）** | ※現在ホーム画面からの導線を一時非表示（将来再開予定）。1コイン/回・1日1回（開始時に確認ポップアップ表示）。小当たり(20%)=コイン×2 / 中当たり(6%)=コイン×6 / 大当たり(0.9%)=コイン×20 / ハズレ(73.1%)=コイン×0（消費分のみ）。当選率26.9%、期待値0.94 |
...
| **スロットキャンペーン設定（一時非表示）** | スロットキャンペーンの開始日・終了日の設定（※現在ホーム画面からの導線を一時非表示） |
