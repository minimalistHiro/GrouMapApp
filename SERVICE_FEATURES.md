...
> **最終更新**: 2026-02-24（ホーム「今日のレコメンド」を1ページ1店舗カルーセルに刷新。最大5件・3秒自動送り（操作中停止）・状態表示「未訪問 / スタンプ X/10 / 満了」に統一し、見出し/カード内文字をコンパクト調整。あわせてミッション導線の黄色基調化、コイン交換一覧の `stores.isOwner=true` 除外、マップ閲覧報酬1日1回制限、メールOTP件名プレフィックス「【ぐるまっぷ】」統一を反映）
>
...
| **プロフィール完成度カード** | アカウント設定画面に2段階で表示。Stage 1: 基本プロフィール7項目（表示名・生年月日・性別・都道府県・市区町村・職業・プロフィール画像）未完成時は「プロフィールを完成させよう」カード。Stage 2: 基本完成後に興味カテゴリ未設定時は「興味カテゴリを設定しよう」カード。全完了でカード非表示。基本プロフィール未完成時は「プロフィール編集」メニューを非表示化 |
...
| **統計カプセルバー** | コイン・バッジ・スタンプの現在数をおすすめ店舗セクション上にカプセル形式で横並び表示（コンパクトな丸角カプセル、左アイコン + 「数値+ラベル」表記。Firestoreリアルタイム取得、バッジ数はローカル定義に存在するバッジのみカウント、未ログイン時は `-` 表示） |
...
| **スタンプカード運用** | 1店舗につき1枚のみ運用（達成後の2枚目以降は発行しない） |
| **スタンプ達成特典** | スタンプ達成時の特典（**値引型のみ**。例: 「○○円引き」。達成に必要なスタンプ数は店舗が1〜10個の範囲で自由設定） |
| **常連向けクーポン** | 廃止（常連向けクーポンは提供しない） |
...
| **新規登録ミッション** | 初回限定の5種ミッション（プロフィール完成・マップ初利用・お気に入り登録・店舗詳細閲覧・スタンプ初獲得）、合計最大15コイン。すべて受取完了するまでデイリー/ログインタブはUI上ロックし、バックエンドでもデイリーミッション達成処理・ログインストリーク更新を停止（ボタン通知の誤点灯を防止） |
