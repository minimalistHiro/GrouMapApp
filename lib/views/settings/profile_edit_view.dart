...
class ProfileEditView extends ConsumerStatefulWidget {
  ...
  const ProfileEditView({super.key, this.showNextButton = false});
  ...
  Future<void> _loadCurrentUserData() async {
    try {
      ...
      if (doc != null && doc.exists) {
        ...
        _selectedGender = (data['gender'] as String?)?.isNotEmpty == true
            ? data['gender']
            : null;
        _selectedPrefecture =
            (data['prefecture'] as String?)?.isNotEmpty == true
                ...
                : null;
        _selectedCity =
            (data['city'] as String?)?.isNotEmpty == true ? data['city'] : null;

        // 新規フィールド読み込み
        _usernameController.text = (data['username'] ?? '').toString();
        _currentUsername = _usernameController.text.isNotEmpty
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (newUsername.isNotEmpty && newUsername != _currentUsername) {
        ...
        final existingDoc =
            await firestore.collection('usernames').doc(usernameLower).get();
        if (existingDoc.exists && existingDoc.data()?['uid'] != user?.uid) {
          if (!mounted) return;
          setState(() => _isSaving = false);
          await _showErrorDialog(
            'このユーザーIDは既に使用されています。別のユーザーIDをお試しください。',
          );
          return;
        }
      ...
      if (user != null) {
        final displayName =
            isGoogleUser ? (user.displayName ?? '') : newDisplayName;
        if (displayName.isNotEmpty &&
          ...
          MissionService()
              .markRegistrationMission(user.uid, 'profile_completed');
        }
      }
      ...
      setState(() => _isSaving = false);
      await _showErrorDialog(
        'プロフィールの更新に失敗しました。時間をおいてもう一度お試しください。',
      );
    } finally {
                      ...
                      children: [
                          ...
                          Center(
                            child: _isUploadingImage
                                ...
                                : IconImagePickerField(
                                    ...
                                    showRemove: (_selectedImageBytes != null) ||
                        ...
                        SizedBox(
                          ...
                          child: CustomButton(
                            ...
                            onPressed:
                                (_isSaving || _isUploadingImage) ? null : _save,
