...
description: 「画像を生成して」「画像を作って」「アイコンを作って」「バナーを作って」「ポスターを作成して」など、画像生成の依頼時に、Nano Banana Pro（Gemini 3 Pro Image）APIを使って画像を自動生成する。
---
...
1. ユーザーの依頼内容を分析し、**英語の詳細なプロンプト**を作成する。
   ...
   - 「ポスターを作成して」の依頼では、`/Users/kanekohiroki/Desktop/groumapapp/groumap_poster.png` のテイストに合わせる指示と、ユーザーのテキスト要望を明示的に含める。
   - 「ポスターを作成して」の依頼では、下部のアプリダウンロード部分（QR・ストアバッジ・下部帯）を**編集せずそのまま維持する**指示を必ず入れる。
...
- 参照元ポスター: `/Users/kanekohiroki/Desktop/groumapapp/groumap_poster.png`
- 目的: 参照元ポスターのテイストに合わせつつ、ユーザー指定テキストに差し替えたポスターを生成する。
- 下部固定ルール: 下部のアプリダウンロード部分（QR・ストアバッジ・下部帯）は編集禁止。元ポスターと同一の見た目で出力する。
- 生成後チェック: 下部が少しでも変わっていたら、プロンプトを強化して再生成する。

ポスター用プロンプトに必ず入れる英語指示例:
`"Create a poster in the same visual taste as the reference GrouMap poster. Apply the user's requested text to the main message area. Keep the bottom app download section exactly identical to the original poster, including QR code area, store badges, and footer layout. Do not modify or redesign that bottom section."`
